    ...
    + super::create_pos::CreatePosModule
{
    ...
    fn create_pos_from_lp(&self, steps: StepsToPerform) -> DctTokenPayment {
        require!(
        ...
        if matches!(steps, StepsToPerform::EnterFarm) {
            self.send()
                .direct_non_zero_dct_payment(&caller, &farm_tokens);
        ...
        self.send()
            .direct_non_zero_dct_payment(&caller, &ms_tokens);
